####################################
#
# Stepwise resutls from heart_stepwise_folds.R
#
####################################
#           Nvar      AUC
# Fold 1     11    0.9322199
...
for (i in 1:kfolds) {
  ...
  for (j in 1:nvarH) {                             
    a <- paste(".../KFoldsCV/Heart/LR/Fold", i, sep = "")
    a <- paste(a, "/LR_results_heart_iv_", sep = "")
    a <- paste(a, j, sep="")
    a <- paste(a, ".rds", sep="")
#    print(a)
    results_heart <- readRDS(a)
    
    aucValueTrain<-AUC(results_heart$predic_train, results_heart$target_train)
    aucValueTest<-AUC(results_heart$predic_test, results_heart$target_test)
...
#  View(auc_result_heart)
  ...
  gTrain <- gTrain +  coord_fixed(ratio = 5) +
    ...
    annotate("curve", x = 2, y = 0.29, xend = 4.5, yend = 0.2, curvature = .3,
...
#                                                               Train     Test     
# Fold 1     11    0.9322199              0.921         8       0.921     0.921 
# Fold 2     10    0.93882                0.937         5       0.919     0.81 
# Fold 3      9    0.931933               0.921         5       0.904     0.879  
# Fold 4     10    0.927112               0.924         7       0.907     0.912  
# Fold 5      7    0.9261938              0.914         5       0.908     0.889   
#################################### 
# Average    9.4   0.9312557              0.9234        6       0.9118    0.8822 
####################################
mean(c(11,10,9,10,7))
mean(c(0.9322199,0.93882,0.931933,0.927112,0.9261938))
mean(c(0.921,0.937,0.921,0.924,0.914))
mean(c(8,5,5,7,5))
mean(c(0.921,0.919,0.904,0.907,0.908))
mean(c(0.921,0.81,0.879,0.912,0.889))
