...
for (i in 1:kfolds) {
  ...
  for (j in 1:nvarH) {                             
    a <- paste(".../KFoldsCV/Higgs/SVM/Fold", i, sep = "")
    a <- paste(a, "/SVM_results_higgs_iv_", sep = "")
    a <- paste(a, j, sep="")
    a <- paste(a, ".rds", sep="")
#    print(a)
    results_higgs <- readRDS(a)
    
    aucValueTrain<-AUC(results_higgs$predic_train[, "1"], results_higgs$target_train)
    aucValueTest<-AUC(results_higgs$predic_test[, "1"], results_higgs$target_test)
...
#  View(auc_result_higgs)
  ...
  gTrain <- gTrain +  coord_fixed(ratio = 15) +
    ...
    annotate("label", label = "Number of variables \n from BEM", x = 24, y = 0.4, size = 3, colour = "red" ) +
    #  annotate("text", label = "from BEM", x = 17, y = 0.3, size = 3, colour = "red") +
    annotate("curve", x = 24, y = 0.325, xend = bem_result[i, 1], yend = 0.2, curvature = -.3,
             arrow = arrow(length = unit(2, "mm")), colour = "red") +
    annotate("label", label = "Number of variables \n from our method", x = 7, y = 0.4, size = 3, colour = "magenta" ) +
    #  annotate("text", label = "", x = 4, y = 0.3, size = 3, colour = "magenta" ) +
    annotate("curve", x = 7, y = 0.325, xend = varMet, yend = 0.2, curvature = .3,
             arrow = arrow(length = unit(2, "mm")), colour = "magenta") +
    annotate("label", label = "Threshold using BEM \n and IV ranking", x = 25, y = 0.65, size = 3, colour = "blue" ) +
    #  annotate("text", label = "and IV ranking", x = 25, y = 0.75, size = 3, colour = "blue" ) 
    annotate("curve", x = 25, y = 0.725,  xend = bem_result[i, 1], yend = auc_result_higgs[["aucTrain"]][bem_result[i, 1]], curvature = .1,
...
#                                                               Train     Test     
# Fold 1     18       0.6862255          0.81           12      0.795     0.763
# Fold 2     20       0.6867432          0.812          13      0.796     0.77
# Fold 3     20       0.6860155          0.813          13      0.797     0.761
# Fold 4     17       0.6853733          0.807          12      0.794     0.77  
# Fold 5     18       0.6854524          0.807          12      0.791     0.769   
####################################
# Average    18.6     0.685962           0.8098        12.4     0.7946    0.7666
####################################
mean(c(18,20,20,17,18))
mean(c(0.6862255,0.6867432,0.6860155,0.6853733,0.6854524))
mean(c(0.81,0.812,0.813,0.807,0.807))
mean(c(12,13,13,12,12))
mean(c(0.795,0.796,0.797,0.794,0.791))
mean(c(0.763,0.77,0.761,0.77,0.769))
