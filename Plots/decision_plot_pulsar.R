...
for (i in 1:kfolds) {
  ...
  for (j in 1:nvarH) {                             
    a <- paste(".../KFoldsCV/Pulsar/DT/Fold", i, sep = "")
    a <- paste(a, "/DT_results_pulsar_iv_", sep = "")
    a <- paste(a, j, sep="")
    a <- paste(a, ".rds", sep="")
#    print(a)
    results_pulsar <- readRDS(a)
    
    aucValueTrain<-AUC(results_pulsar$predic_train[, 2], results_pulsar$target_train)
    aucValueTest<-AUC(results_pulsar$predic_test[, 2], results_pulsar$target_test)
...
#                                                               Train     Test
# Fold 1     8       0.9787127           0.995          3       0.978     0.962 
# Fold 2     8       0.9796145           0.995          3       0.979     0.973   
# Fold 3     7       0.979932            0.993          2       0.974     0.963  
# Fold 4     7       0.9800304           0.992          3       0.974     0.969   
# Fold 5     7       0.9791782           0.994          3       0.976     0.971  
####################################
# Average    7.4     0.9794936          0.9938          2.4     0.9762    0.9676   
####################################
mean(c(8,8,7,7,7))
mean(c(0.9787127,0.9796145,0.979932,0.9800304,0.9791782))
mean(c(0.995,0.9950,0.993,0.992,0.994))
mean(c(3,3,0,3,3))
mean(c(0.978,0.979,0.974,0.974,0.976))
mean(c(0.962,0.973,0.963,0.969,0.971))
