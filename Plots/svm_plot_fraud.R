...
for (i in 1:kfolds) {
  ...
  for (j in 2:nvarH) {                             
    a <- paste(".../KFoldsCV/Fraud/SVM/Fold", i, sep = "")
    a <- paste(a, "/SVM_results_fraud_iv_", sep = "")
    a <- paste(a, j, sep="")
    a <- paste(a, ".rds", sep="")
#    print(a)
    results_fraud <- readRDS(a)
    
    aucValueTrain<-AUC(results_fraud$predic_train[, "1"], results_fraud$target_train)
    aucValueTest<-AUC(results_fraud$predic_test[, "1"], results_fraud$target_test)
...
#  View(auc_result_fraud)
  ...
  for (k in 1:(nvarH-1)){
    if (.98*auc_result_fraud[["aucTrain"]][bem_result[i, 1]-1] < auc_result_fraud[["aucTrain"]][k]){
      flagAUC <- flagAUC + 1
      if (flagAUC == 1){
        varMet <- k+1
        print(varMet) 
        ...
        print(auc_result_fraud[["aucTrain"]][bem_result[i, 1]-1])
      }
  ...
  gTrain <- gTrain +  coord_fixed(ratio = 15) +
    ...
    geom_hline(yintercept = auc_result_fraud[["aucTrain"]][bem_result[i, 1]-1] , linetype="dotted", color = "blue", size=1.0) +
...
#                                                                 Train     Test
# Fold 1     29     0.9970249              1            5         0.987     0.984 
# Fold 2     30     0.998279               1            3         0.985     0.937 
# Fold 3     30     0.9975251              1            5         0.99      0.959 
# Fold 4     28     0.9975678              1            5         0.986     0.932
# Fold 5     29     0.9981895              1            5         0.989     0.955  
####################################
# Average    29.2   0.9977173              1            4.6       0.9874    0.9534   
####################################
mean(c(29,30,30,28,29))
mean(c(0.9970249,0.998279,0.9975251,0.9975678,0.9981895))
mean(c(5,3,5,5,5))
mean(c(0.987,0.985,0.99,0.986,0.989))
mean(c(0.984,0.937,0.959,0.932,0.955))  
