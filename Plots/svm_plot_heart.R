...
for (i in 1:kfolds) {
  ...
  for (j in 1:nvarH) {                             
    a <- paste(".../KFoldsCV/Heart/SVM/Fold", i, sep = "")
    a <- paste(a, "/SVM_results_heart_iv_", sep = "")
    a <- paste(a, j, sep="")
    a <- paste(a, ".rds", sep="")
#    print(a)
    results_heart <- readRDS(a)
    
    aucValueTrain<-AUC(results_heart$predic_train[, "1"], results_heart$target_train)
    aucValueTest<-AUC(results_heart$predic_test[, "1"], results_heart$target_test)
...
#                                                               Train     Test
# Fold 1     11    0.9322199             0.967          6       0.951     0.922   
# Fold 2     10    0.93882               0.976          5       0.96      0.853  
# Fold 3      9    0.931933              0.969          6       0.953     0.918  
# Fold 4     10    0.927112              0.963          6       0.944     0.937  
# Fold 5      7    0.9261938             0.959          5       0.946     0.925   
####################################
# Average    9.4   0.9312557             0.9668        5.6      0.9508    0.911     
####################################
mean(c(11,10,9,10,7))
mean(c(0.9322199,0.93882,0.931933,0.927112,0.9261938))
mean(c(0.967,0.976,0.969,0.963,0.959))
mean(c(6,5,6,6,5))
mean(c(0.951,0.96,0.953,0.944,0.946))
mean(c(0.922,0.853,0.918,0.937,0.925))
