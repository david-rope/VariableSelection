...
for (i in 1:kfolds) {
  ...
  for (j in 1:nvarH) {                             
    a <- paste(".../KFoldsCV/Pulsar/LR/Fold", i, sep = "")
    a <- paste(a, "/LR_results_pulsar_iv_", sep = "")
    a <- paste(a, j, sep="")
    a <- paste(a, ".rds", sep="")
#    print(a)
    results_pulsar <- readRDS(a)
    
    aucValueTrain<-AUC(results_pulsar$predic_train, results_pulsar$target_train)
    aucValueTest<-AUC(results_pulsar$predic_test, results_pulsar$target_test)
...
#                                                               Train      Test
# Fold 1     8       0.9787127           0.979          1       0.974     0.975
# Fold 2     8       0.9796145           0.98           1       0.976     0.968 
# Fold 3     7       0.979932            0.98           1       0.976     0.966 
# Fold 4     7       0.9800304           0.98           1       0.974     0.975  
# Fold 5     7       0.9791782           0.979          1       0.974     0.976 
#################################### 
# Average    7.4     0.9794936           0.9796         1       0.9748    0.972 
####################################
mean(c(8,8,7,7,7))
mean(c(0.9787127,0.9796145,0.979932,0.9800304,0.9791782))
mean(c(0.979,0.98,0.98,0.98,0.979))
mean(c(1,1,1,1,1))
