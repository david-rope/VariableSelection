...
noheart <- data_train[data_train$target == "0", ]
heart <- data_train[data_train$target == "1", ]
nrow(data_train[data_train$target == "0", ])
nrow(data_train[data_train$target == "1", ])
...
set.seed(10)
kfolds <- 5
noheart_folds <- split(noheart, sample(rep(1:kfolds)))
heart_folds <- split(heart, sample(rep(1:kfolds)))

nf <- c(1:kfolds)
for (i in 1:kfolds) {
   class_train_0 <- paste("df_noheart_train_fold_", i, sep = "")
   class_train_1 <- paste("df_heart_train_fold_", i, sep = "")
   class_test_0 <- paste("df_noheart_test_fold_", i, sep = "")
   class_test_1 <- paste("df_heart_test_fold_", i, sep = "")
   # test sample -> 20%
   new_df_test_0 <- assign(class_test_0, noheart_folds[[i]])
   new_df_test_1 <- assign(class_test_1, heart_folds[[i]])
   ...
   for (j in nf[-i]){
     new_df_train_0 <- rbind(new_df_train_0, noheart_folds[[j]]) 
     new_df_train_1 <- rbind(new_df_train_1, heart_folds[[j]]) 
   }
...
for (i in 1:kfolds) {
   ...
   n_class_0 <- nrow(df_train_base[df_train_base$target == "0", ])
   n_class_1 <- nrow(df_train_base[df_train_base$target == "1", ])
   data_smote <- data.frame()
   if (n_class_0 < n_class_1){
      ds <- as.integer(n_class_1/n_class_0)
      data_smote <- SMOTE(df_train_base, target = df_train_base$target,
                          K = 5, dup_size = ds)
