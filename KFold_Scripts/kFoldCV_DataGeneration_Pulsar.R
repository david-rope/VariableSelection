...
nopulsar <- data_train[data_train$target_class == "0", ]
pulsar <- data_train[data_train$target_class == "1", ]
nrow(data_train[data_train$target_class == "0", ])
nrow(data_train[data_train$target_class == "1", ])
...
set.seed(10)
kfolds <- 5
nopulsar_folds <- split(nopulsar, sample(rep(1:kfolds)))
pulsar_folds <- split(pulsar, sample(rep(1:kfolds)))

nf <- c(1:kfolds)
for (i in 1:kfolds) {
   class_train_0 <- paste("df_nopulsar_train_fold_", i, sep = "")
   class_train_1 <- paste("df_pulsar_train_fold_", i, sep = "")
   class_test_0 <- paste("df_nopulsar_test_fold_", i, sep = "")
   class_test_1 <- paste("df_pulsar_test_fold_", i, sep = "")
   # test sample -> 20%
   new_df_test_0 <- assign(class_test_0, nopulsar_folds[[i]])
   new_df_test_1 <- assign(class_test_1, pulsar_folds[[i]])
   ...
   for (j in nf[-i]){
     new_df_train_0 <- rbind(new_df_train_0, nopulsar_folds[[j]]) 
     new_df_train_1 <- rbind(new_df_train_1, pulsar_folds[[j]]) 
   }
...
nrow(new_df_train_0[new_df_test_0$target_class == "0", ])
nrow(new_df_train_1[new_df_test_1$target_class == "1", ])
nrow(new_df_test_0[new_df_test_0$target_class == "0", ])
nrow(new_df_test_1[new_df_test_1$target_class == "1", ])
#####################################
##
...
for (i in 1:kfolds) {
   ...
   n_class_0 <- nrow(df_train_base[df_train_base$target_class == "0", ])
   n_class_1 <- nrow(df_train_base[df_train_base$target_class == "1", ])
   data_smote <- data.frame()
   if (n_class_0 < n_class_1){
      ds <- as.integer(n_class_1/n_class_0)
      data_smote <- SMOTE(df_train_base, target = df_train_base$target_class,
                          K = 5, dup_size = ds)
