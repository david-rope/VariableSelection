...
dt.model <- C5.0.formula(target_class ~
, data = data_train)
...
print(total_time)
results <- list(target_train = data_train$target_class, predic_train = predic.train,
                target_test = data_test$target_class, predic_test = predic.test, 
                tiempo = total_time)

saveRDS(results,
        "/data1/David/OutputR/KFoldsCV/Pulsar/DT/Fold5/DT_results_pulsar_iv_NVAR.rds")
saveRDS(dt.model, "/data1/David/OutputR/KFoldsCV/Pulsar/DT/Fold5/DT_model_pulsar_iv_NVAR.rds")
