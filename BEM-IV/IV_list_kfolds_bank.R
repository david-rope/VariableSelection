...
totalIV <- matrix()
for (i in 1:kfolds) { 
  file_name <- paste("/home/david/R/PaperR/TestData/KFoldsCVSMOTE/Bank/DataSets/train_smote_bank_fold_", i, sep = "")
...
#  View(data_train)
  ...
  for(j in plist) {       # for-loop over columns
    print(j) 
    resultWOE <- woe.binning(data_train,'y', j, 
...
for (i in 1:kfolds) { 
  fileOutputIV <- paste("/home/david/R/PaperR/TestData/KFoldsCVSMOTE/Bank/IVRank/IV_bank_fold_", i, sep = "")
