...
#  View(data_train)
  ...
  plist<-plist[plist != "target_class"];
  ...
  for(j in plist) {       # for-loop over columns
    print(j) 
    resultWOE <- woe.binning(data_train,'target_class', j, 
...
for (i in 1:kfolds) { 
  fileOutputIV <- paste("/home/david/R/PaperR/TestData/KFoldsCVSMOTE/Pulsar/IVRank/IV_pulsar_fold_", i, sep = "")
