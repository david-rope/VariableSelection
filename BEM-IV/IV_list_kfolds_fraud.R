...
totalIV <- matrix()
for (i in 1:kfolds) { 
  file_name <- paste(".../Fraud/DataSets/train_smote_fraud_fold_", i, sep = "")
...
#  View(data_train)
  ...
  for(j in plist) {       # for-loop over columns
    print(j) 
    resultWOE <- woe.binning(data_train,'Class', j, 
...
for (i in 1:kfolds) { 
  fileOutputIV <- paste(".../Fraud/IVRank/IV_fraud_fold_", i, sep = "")
